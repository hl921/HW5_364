...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/haeleeHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    newPriority = StringField("What is the new priority of the item?", validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        items.priority = form.newPriority.data
        db.session.commit()
...
def delete(lst):
    tdl = TodoList.query.filter_by(title=lst).first()
    db.session.delete(tdl)
    db.session.commit()
